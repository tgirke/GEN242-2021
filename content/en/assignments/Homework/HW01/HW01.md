...
on result page click first entry in BLAST hit list (here [3K9V_A](https://www.ncbi.nlm.nih.gov/protein/3K9V_A?report=genbank&log$=protalign&blast_rank=1&RID=6BZUZS51016)); then select 'Identify Conserved Domains' on side bar; click blue bar labelled 'CYP24A1'; then select 'Interactive View' which will download 'cd20645.cn3' file.
	...
	2. View 3D structure in Cn3D, save structure (screen shot) and highlight heme binding cysteine. Note, Cn3D can be downloaded from [here](https://www.ncbi.nlm.nih.gov/Structure/CN3D/cn3d.shtml). If there are problems with the install of Cn3D, then please use the online only alternative: (i) click on the [3K9V_A](https://www.ncbi.nlm.nih.gov/protein/3K9V_A?report=genbank&log$=protalign&blast_rank=1&RID=6BZUZS51016) page _'Protein 3D Structure'_ instead of _'Identify Conserved Domains'_; (ii) choose one of the two structure entries provided on the subsequent page; (iii) click "full-featured 3D viewer" in the bottom right corner of the structure image; (iv) select the '_Details'_ tab on the right; (v) this shows the structure of the protein on the left and the underlying protein sequence on the right; (vi) highlight the heme binding cysteine in the structure by selecting it in the sequence; (vii) save the structure view to a PNG file or take a screenshot.  
