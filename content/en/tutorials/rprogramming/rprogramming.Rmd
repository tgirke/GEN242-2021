...
(i) Retrieve row labels (genes) that match the query from the previous step, and store them in a 
a `list`.  

```{r id_result_list1, eval=FALSE}
matchingIDlist <- sapply(colnames(queryResult), function(x) names(queryResult[queryResult[ , x] , x]), simplify=FALSE)
matchingIDlist
```

(ii) Return all row labels (genes) that match the above query in a specified number of columns (here 3).
